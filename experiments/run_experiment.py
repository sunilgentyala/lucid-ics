...
from lucid_ics.pipeline import run_pipeline  # noqa: E402
from lucid_ics.simulator import SimulationConfig  # noqa: E402

FAMILIES = ["normal", "unauthorized_write", "replay", "sensor_spoofing", "recon_scan", "dos_flood"]
...
def main():
    ...
    sample = [r for r in records if r.attack_type != "normal"][:6]
    ...
    ax.set_title("LUCID-ICS attack-family confusion matrix (held-out test windows)")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(fig_dir, "confusion_matrix.png"), dpi=180)
    plt.close(fig)

    # ROC curve (binary attack vs normal, risk_score as decision function)
    fpr, tpr, _ = roc_curve(y_true_bin, result.risk_score)
    fig, ax = plt.subplots(figsize=(5.4, 4.6))
    ax.plot(fpr, tpr, lw=2, label="LUCID-ICS risk score")
    ax.plot([0, 1], [0, 1], "--", color="gray", lw=1, label="chance")
    ax.set_xlabel("False positive rate")
    ax.set_ylabel("True positive rate")
    ax.set_title("ROC -- binary attack detection")
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(fig_dir, "roc_curve.png"), dpi=180)
    plt.close(fig)
    ...
    ax.set_title("Global feature importance -- attack-family classifier")
    fig.tight_layout()
    fig.savefig(os.path.join(fig_dir, "feature_importance.png"), dpi=180)
    plt.close(fig)
    ...
    ax.set_xlabel("LUCID-ICS risk score")
    ax.set_ylabel("Window count")
    ax.set_title("Risk-score separation between normal and attack windows")
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(fig_dir, "risk_score_distribution.png"), dpi=180)
    plt.close(fig)
